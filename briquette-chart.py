...
def main():
    ...
    # Auto-restore from GitHub if fewer than 3 entries
    if len(prices) < 3:
        ...
        if restore_result.returncode == 0 and os.path.exists("/tmp/briquette-restore/briquette_prices.json"):
            with open("/tmp/briquette-restore/briquette_prices.json") as f:
            ...
            prices = historical
            save_prices(prices)
            print(f"RESTORED: {len(prices)} points from GitHub", file=sys.stderr)
        else:
