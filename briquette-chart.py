...
def generate_svg(prices, width=600, height=400):
    ...
    if not prices:
        return '<svg xmlns="http://www.w3.org/2000/svg" width="600" height="400"><rect width="600" height="400" fill="#1a1a2e"/><text x="50%" y="50%" text-anchor="middle" fill="#fff">No data</text></svg>'
    padding = 60
    chart_w = width - 2 * padding
    chart_h = height - 2 * padding
    all_prices = [p["price"] for p in prices]
    ...
    return f'<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">\n  <rect width="{width}" height="{height}" fill="#1a1a2e"/>\n  {polyline}\n  {dot}\n  <text x="{width/2}" y="25" text-anchor="middle" fill="#e94560" font-size="14" font-weight="bold">Briquette Price (€/t)</text>\n</svg>'


def restore_from_github():
    """Auto-restore price data from GitHub if local data is sparse (< 3 entries)."""
    result = subprocess.run(
        ["git", "clone", "https://github.com/spidertje/briquette-prices.git", "/tmp/briquette-restore"],
        capture_output=True, timeout=30
    )
    if result.returncode != 0:
        print("ERROR: git clone failed for restore.", file=sys.stderr)
        return False
    restore_file = "/tmp/briquette-restore/briquette_prices.json"
    if not os.path.exists(restore_file):
        print("ERROR: no price data in remote repo.", file=sys.stderr)
        return False
    try:
        with open(restore_file) as f:
            historical = json.load(f)
    except (json.JSONDecodeError, OSError):
        print("ERROR: could not read remote price data.", file=sys.stderr)
        return False
    local_data = load_prices() if os.path.exists(PRICE_FILE) else []
    combined = list(historical)
    for entry in local_data:
        if not any(str(e.get('date', '')).strip() == str(entry.get('date', '')).strip() for e in historical):
            combined.append(entry)
    save_prices(combined)
    print(f"RESTORED: {len(combined)} points from GitHub", file=sys.stderr)
    return True
...
def main():
    ...
    if len(prices) < 3:
        print(f"WARNING: Only {len(prices)} entries, attempting GitHub restore...", file=sys.stderr)
        if not restore_from_github():
            print("WARNING: Could not restore from GitHub, continuing with local data.", file=sys.stderr)
        prices = load_prices()
